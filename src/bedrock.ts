...
export interface BedrockBlockState { readonly name: string; readonly states?: Readonly<Record<string, BedrockStateValue>>; }
export type UnsupportedBlockPolicy = "barrier" | "air" | "throw";
export interface ConvertBedrockOptions { readonly unsupportedBlockPolicy?: UnsupportedBlockPolicy; }

const NAME_ALIASES: Readonly<Record<string, string>> = {
  "minecraft:grass": "minecraft:grass_block", "minecraft:brick_block": "minecraft:bricks",
  "minecraft:lit_furnace": "minecraft:furnace", "minecraft:standing_sign": "minecraft:oak_sign",
  "minecraft:wall_sign": "minecraft:oak_wall_sign", "minecraft:wooden_door": "minecraft:oak_door",
  "minecraft:trapdoor": "minecraft:oak_trapdoor", "minecraft:waterlily": "minecraft:lily_pad",
};
const STAIR_DIRECTIONS: Readonly<Record<number, string>> = { 0: "east", 1: "west", 2: "south", 3: "north" };
const CARDINAL_DIRECTIONS: Readonly<Record<number, string>> = { 0: "south", 1: "west", 2: "north", 3: "east" };
...
function bool(value: BedrockStateValue | undefined): boolean { return value === true || value === 1 || value === "1" || value === "true"; }
function text(states: Readonly<Record<string, BedrockStateValue>>, ...keys: string[]): string | undefined {
  for (const key of keys) { const value = states[key]; if (value !== undefined) return String(value); }
  return undefined;
}
...
export function convertBedrockBlockState(block: BedrockBlockState, options: ConvertBedrockOptions = {}): JavaBlockState {
  const policy = options.unsupportedBlockPolicy ?? "barrier";
  const rawName = block.name.includes(":") ? block.name : `minecraft:${block.name}`;
  if (!rawName.startsWith("minecraft:")) return unsupported(block, policy);
  const name = namespaced(rawName);
  ...
  if (name.endsWith("_pressure_plate")) return { name, properties: { powered: String(Number(states.redstone_signal ?? 0) > 0) } };
